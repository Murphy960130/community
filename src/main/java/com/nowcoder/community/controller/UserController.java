...
public class UserController implements CommunityConstant {
    ...
    public String updatePassword(String oldPassword, String newPassword, String newPasswordConfirm, Model model) {
        ...
        if(StringUtils.isBlank(newPasswordConfirm)) {
            model.addAttribute("confirmPasswordError", "确认新密码不能为空！");
            return "/site/setting";
        }
    ...
    public String getMyPosts(@PathVariable("userId") int userId, Page page, Model model) {
        ...
        // 该用户的帖子
        List<DiscussPost> list = discussPostService.findDiscussPosters(userId, page.getOffset(), page.getLimit());
    ...
    public String getMyReplies(@PathVariable("userId") int userId, Page page, Model model) {
        ...
        // 该用户回复帖子的个数
        int replyCount = commentService.findCommentCountByUserId(userId, 1);
        ...
        // 该用户的回复
        List<Comment> list = commentService.findCommentsByUserId(userId, 1, page.getOffset(), page.getLimit());
